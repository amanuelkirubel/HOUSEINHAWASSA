...
const routes = [
  { method:'GET', re:/^\/api\/listings$/, handler: async (req, res, url) => {
    ...
    send(res, 200, await db.getListings({
  ...
  { method:'GET', re:/^\/api\/listings\/([^/]+)$/, handler: async (req, res, url, m) => {
    const listing = await db.getListingById(m[1]);
  ...
  { method:'POST', re:/^\/api\/listings$/, handler: async (req, res) => {
    ...
    try{
      const listing = await db.createListing('guest', body);
      send(res, 200, { listing });
  ...
  { method:'POST', re:/^\/api\/admin\/register$/, handler: async (req, res) => {
    const body = await readBody(req);
    try{
      await db.createUser({ username: body.email, password: body.password });
      const token = await db.createSession(String(body.email||'').toLowerCase().trim());
      send(res, 200, { token });
    }catch(e){ send(res, e.status || 500, { error: e.message || 'Something went wrong.' }); }
  }},
  { method:'POST', re:/^\/api\/admin\/login$/, handler: async (req, res) => {
    const body = await readBody(req);
    const user = await db.verifyUser(String(body.email||'').toLowerCase().trim(), body.password);
  ...
  { method:'PATCH', re:/^\/api\/listings\/([^/]+)$/, handler: async (req, res, url, m) => {
    const admin = await getAdminFromRequest(req);
    const body = await readBody(req);
    if(typeof body.title === 'string') body.title = stripContactInfo(body.title);
    if(typeof body.description === 'string') body.description = stripContactInfo(body.description);
    try{
      const listing = await db.updateListing(m[1], { editToken: body.edit_token, isAdmin: !!admin }, body);
      send(res, 200, { listing });
    }catch(e){ send(res, e.status || 500, { error: e.message || 'Something went wrong.' }); }
  }},
  { method:'PATCH', re:/^\/api\/listings\/([^/]+)\/status$/, handler: async (req, res, url, m) => {
    ...
    try{
      const listing = await db.updateListingStatus(m[1], status);
      send(res, 200, { listing });
    }catch(e){ send(res, e.status || 500, { error: e.message || 'Something went wrong.' }); }
  }},
];
