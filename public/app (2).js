(function(){
  ...
  let state = {
    ...
    authMode:'login', authError:'', sellError:'',
    toast:'', loading:true,
  };
  ...
  async function doSignup(username, password, phone, city){
    try{
      const data = await api('/api/auth/signup', { method:'POST', body: JSON.stringify({username,password,phone,city}) });
      localStorage.setItem('hih_token', data.token);
      setState({ currentUser:data.user, view:'home', authError:'' });
  ...
  function renderAuth(){
    return `<div class="wrap"><div class="authbox">
      <h2>Log in</h2>
      <p class="sub">Welcome back to House in Hawassa.</p>
      <form data-form="auth">
  ...
  async function render(){
    ...
    else if(state.view==='login') body = renderAuth();
  ...
  function attachEvents(){
    ...
    if(sellForm) sellForm.addEventListener('submit', async (e)=>{
      ...
      await createListing({ title: cleanTitle, photo_data: photoData, city: fd.get('city'), price: fd.get('price'), bedrooms: fd.get('bedrooms'), bathrooms: fd.get('bathrooms'), area: fd.get('area'), description: cleanDesc });
